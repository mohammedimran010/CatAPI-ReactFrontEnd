...
import { setLoading, setError, setImages, setFavourites, setVotes, setRedirect } from '../redux/slices/catsSlice';
...
export const getImages = () => async(dispatch) => {
    ...
    try {
        const response = await axios.get(`${IMAGES_ENDPOINT}/?limit=10`, config);
        dispatch(setImages(await response.data));
...
export const toggleVote = (selectedImage, value) => async(dispatch) => {
    ...
    try {
        await axios.post(VOTES_ENDPOINT, {image_id: selectedImage, value}, config);
        dispatch(getVotes());
    } catch (err) {
        dispatch(setError(err.response.data.message));
    }
